...
def process_table(table_path: str, empty_value: str, properties: dict[str, dict], table_data_column_nr: int) -> dict[
    ...
    for k in range(1, tab.shape[0]):
        row = tab.iloc[k]
        guid = row[0]
        for i in range(1, table_data_column_nr):  
...
def capacity_propagation_ventilation(file_path, table_path):
    '''
    Implementation function based on the propagation of capacity value of air terminals given in table to all connected entities targeting the Rooftop Unit
    '''
